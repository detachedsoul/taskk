...
const Searchbar = ({isActive}: component): JSX.Element => {
    return (
        <form className={`fixed border bg-neutral-900 text-neutral-400 border-neutral-700 rounded-lg w-[90%] left-[5%] px-3 ${isActive ? 'bottom-[calc(100%-8rem)]' : '-bottom-full'} transition-all ease-in-out duration-500 lg:hidden`}>
            <label className="flex items-center gap-2" htmlFor="mobile-search">
                <SearchIcon strokeWidth={1} />

                <input className="bg-inherit py-3 outline-none w-full focus:text-white focus:placeholder:text-white" type="search" name="search" id="mobile-search" placeholder="Search" />
